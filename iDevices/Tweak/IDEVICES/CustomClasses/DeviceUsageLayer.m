...
#import "DeviceUsageLayer.h"

@implementation DeviceUsageLayer
@dynamic value;
@dynamic maxValue;
...
- (void)drawEmptyBar:(CGRect)rect context:(CGContextRef)c{
  ...
  if (self.progressAppearanceType == DeviceUsageAppearanceTypeOverlaysEmptyLine) {
    radius = radius - MAX(self.emptyLineWidth, self.progressLineWidth)/2.f;
  } else if (self.progressAppearanceType == DeviceUsageAppearanceTypeAboveEmptyLine) {
