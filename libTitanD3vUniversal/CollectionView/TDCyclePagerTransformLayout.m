...
typedef NS_ENUM(NSUInteger, TDTransformLayoutItemDirection) {
    TDTransformLayoutItemLeft,
    TDTransformLayoutItemCenter,
    TDTransformLayoutItemRight,
};


@interface TDCyclePagerTransformLayout () {
    struct {
...
@interface TDCyclePagerViewLayout ()
...
@implementation TDCyclePagerTransformLayout
...
- (void)setDelegate:(id<TDCyclePagerTransformLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(TDCyclePagerViewLayout *)layout {
    _layout = layout;
...
- (TDTransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    TDTransformLayoutItemDirection direction= TDTransformLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = TDTransformLayoutItemCenter;
    }else if (centerX - contentCenterX < 0) {
        direction = TDTransformLayoutItemLeft;
    }
    return direction;
}
...
{
    return _layout.layoutType == TDCyclePagerTransformLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != TDCyclePagerTransformLayoutNormal) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    }else if(_layout.layoutType != TDCyclePagerTransformLayoutNormal){
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(TDCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case TDCyclePagerTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case TDCyclePagerTransformLayoutCoverflow:
        {
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(TDCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case TDCyclePagerTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        TDTransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case TDTransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width*(1-scale)/2;
                break;
            case TDTransformLayoutItemRight:
                translate = -1.15 * attributes.size.width*(1-scale)/2;
                break;
            default:
                // center
...
@implementation TDCyclePagerViewLayout
