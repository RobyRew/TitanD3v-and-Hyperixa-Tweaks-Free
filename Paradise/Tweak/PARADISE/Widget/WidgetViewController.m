...
-(instancetype)initWithHash:(NSString*)imgHash imgTitle:(NSString*)imgTitle iconView:(SBIconView*)iconView{
  ...
  NSLog(@"ParadiseDebug initWithHash self.imgHash:-%@, iconView:-%@" ,self.imgHash, iconView.iconImageSnapshot);
  return self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (indexPath.row == 1) {
    ...
    WidgetCell *cell = [tableView dequeueReusableCellWithIdentifier:@"iconCell"];

    if (cell == nil) {
      cell = [[WidgetCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"iconCell"];
    }
