...
- (void)setScrollDirection:(TDMarqueeLabelDirection)direction {
...
- (void)scrollLabelIfNeeded {
  ...
  BOOL doScrollLeft = (self.scrollDirection == TDMarqueeLabelDirectionLeft);
...
- (void)applyGradientMaskForFadeLength:(CGFloat)fadeLength enableFade:(BOOL)fade {
  ...
  if (fadeLength) {
    ...
    if (!fade) switch (self.scrollDirection) {
      case TDMarqueeLabelDirectionLeft:
      leftFadePoint = @0;
      break;

      case TDMarqueeLabelDirectionRight:
      leftFadePoint = @0;
      rightFadePoint = @1;
      break;
    }
