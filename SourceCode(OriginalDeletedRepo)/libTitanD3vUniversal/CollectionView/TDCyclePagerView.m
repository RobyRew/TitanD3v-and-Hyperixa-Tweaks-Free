...
NS_INLINE BOOL TDEqualIndexSection(TDIndexSection indexSection1,TDIndexSection indexSection2) {
    return indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section;
}

NS_INLINE TDIndexSection TDMakeIndexSection(NSInteger index, NSInteger section) {
    TDIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface TDCyclePagerView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, TDCyclePagerTransformLayoutDelegate> {
    struct {
...
@property (nonatomic, assign) TDIndexSection beginDragIndexSection;
...
@implementation TDCyclePagerView
...
- (void)addCollectionView {
    TDCyclePagerTransformLayout *layout = [[TDCyclePagerTransformLayout alloc]init];
...
- (void)timerFired:(NSTimer *)timer {
    ...
    [self scrollToNearlyIndexAtDirection:TDPagerScrollDirectionRight animate:YES];
}

#pragma mark - getter

- (TDCyclePagerViewLayout *)layout {
    if (!_layout) {
...
- (void)setDelegate:(id<TDCyclePagerViewDelegate>)delegate {
    _delegate = delegate;
    ...
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((TDCyclePagerTransformLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<TDCyclePagerViewDataSource>)dataSource {
...
- (void)scrollToNearlyIndexAtDirection:(TDPagerScrollDirection)direction animate:(BOOL)animate {
    TDIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
...
- (void)scrollToItemAtIndex:(NSInteger)index animate:(BOOL)animate {
    ...
    if (!_isInfiniteLoop) {
        [self scrollToItemAtIndexSection:TDMakeIndexSection(index, 0) animate:animate];
        return;
    }

    [self scrollToItemAtIndexSection:TDMakeIndexSection(index, index >= self.curIndex ? _indexSection.section : _indexSection.section+1) animate:animate];
}
...
- (void)updateLayout {
    ...
    ((TDCyclePagerTransformLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (BOOL)isValidIndexSection:(TDIndexSection)indexSection {
...
- (TDIndexSection)nearlyIndexPathAtDirection:(TDPagerScrollDirection)direction{
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (TDIndexSection)nearlyIndexPathForIndexSection:(TDIndexSection)indexSection direction:(TDPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }
    
    if (!_isInfiniteLoop) {
        if (direction == TDPagerScrollDirectionRight && indexSection.index == _numberOfItems - 1) {
            return _autoScrollInterval > 0 ? TDMakeIndexSection(0, 0) : indexSection;
        } else if (direction == TDPagerScrollDirectionRight) {
            return TDMakeIndexSection(indexSection.index+1, 0);
        }
        
        if (indexSection.index == 0) {
            return _autoScrollInterval > 0 ? TDMakeIndexSection(_numberOfItems - 1, 0) : indexSection;
        }
        return TDMakeIndexSection(indexSection.index-1, 0);
    }
    
    if (direction == TDPagerScrollDirectionRight) {
        if (indexSection.index < _numberOfItems-1) {
            return TDMakeIndexSection(indexSection.index+1, indexSection.section);
        }
        if (indexSection.section >= kPagerViewMaxSectionCount-1) {
            return TDMakeIndexSection(indexSection.index, kPagerViewMaxSectionCount-1);
        }
        return TDMakeIndexSection(0, indexSection.section+1);
    }
    ...
    if (indexSection.section <= 0) {
        return TDMakeIndexSection(indexSection.index, 0);
    }
    return TDMakeIndexSection(_numberOfItems-1, indexSection.section-1);
}

- (TDIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
        return TDMakeIndexSection(0, 0);
    }
    ...
    return TDMakeIndexSection(curIndex, curSection);
}

- (CGFloat)caculateOffsetXAtIndexSection:(TDIndexSection)indexSection{
    if (_numberOfItems == 0) {
        return 0;
    }
    UICollectionViewFlowLayout *layout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
...
- (void)resetPagerViewAtIndex:(NSInteger)index {
    ...
    [self scrollToItemAtIndexSection:TDMakeIndexSection(index, _isInfiniteLoop ? kPagerViewMaxSectionCount/3 : 0) animate:NO];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_delegate respondsToSelector:@selector(pagerView:didSelectedItemCell:atIndexSection:)]) {
        [_delegate pagerView:self didSelectedItemCell:cell atIndexSection:TDMakeIndexSection(indexPath.item, indexPath.section)];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    TDIndexSection newIndexSection =  [self caculateIndexSectionWithOffsetX:scrollView.contentOffset.x];
    ...
    TDIndexSection indexSection = _indexSection;
    ...
    if (_delegateFlags.didScrollFromIndexToNewIndex && !TDEqualIndexSection(_indexSection, indexSection)) {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if (fabs(velocity.x) < 0.35 || !TDEqualIndexSection(_beginDragIndexSection, _indexSection)) {
        targetContentOffset->x = [self caculateOffsetXAtIndexSection:_indexSection];
        return;
    }
    TDPagerScrollDirection direction = TDPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = TDPagerScrollDirectionLeft;
    }
    TDIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
#pragma mark - TDCyclePagerTransformLayoutDelegate
...
- (void)dealloc {
    ((TDCyclePagerTransformLayout *)_collectionView.collectionViewLayout).delegate = nil;
